...
namespace E_commerce.Presentation
{
    public partial class AdminOrderForm : Form
    {
        ...
        private readonly ILogger<AdminOrderForm> _logger;
        ...
        public AdminOrderForm()
        {
            ...
            // Adjust layout for embedding with dynamic sizing
            this.Padding = new Padding(20);
        }

        public AdminOrderForm(IOrderService orderService, IProductServices productService, ILogger<AdminOrderForm> logger = null) : this()
        {
        ...
        public AdminOrderForm(IOrderService orderService, IProductServices productService, IUserServices userServices, ICartItemService cartItemService) : this()
        {
            _orderService = orderService ?? throw new ArgumentNullException(nameof(orderService));
            _productServices = productService ?? throw new ArgumentNullException(nameof(productService));
        ...
        // Method to adjust the form size dynamically based on the parent form
        public void AdjustSizeToParent(Form parentForm)
        {
            if (parentForm == null) return;

            // Calculate available space: parent width minus sidebar (approx 300px) and margins
            int availableWidth = parentForm.ClientSize.Width - 300 - 40; // Sidebar width + margins
            int availableHeight = parentForm.ClientSize.Height - 40; // Account for top/bottom margins

            // Set the form size
            this.Size = new Size(availableWidth, availableHeight);
            this.Location = new Point(300, 20); // Keep the same position as set in AdminDashboard
            ...
            // Adjust DataGridView size to fit the new form dimensions
            dataGridViewOrders.Location = new Point(20, 100);
            dataGridViewOrders.Size = new Size(this.ClientSize.Width - 40, this.ClientSize.Height - 140);
        }

        private async void AdminOrderForm_Load(object sender, EventArgs e)
        {
            // Populate status combo box with enum values
            statusComboBox.Items.Add("All"); // Option to show all orders
        ...
        private async void searchButton_Click(object sender, EventArgs e)
        {
            ...
            {
                await LoadOrders(); // Load all orders if no status selected
            }
            else if (statusComboBox.SelectedItem.ToString() == "All")
            {
                await LoadOrders(); // Load all orders
            }
            else
            {
